...
请用 **严格的 JSON** 格式输出，必须是有效的 JSON 对象：
...
- **issues**: 问题数组，每个问题包含：
  ...
  - **endLine**: 结束行号（数字，可选）
  - **fixPrompt**: 英文修复指令，简短明确
  - **suggestion**: 具体的代码修改建议（可选）

### 重要提醒：

1. 输出必须是有效的 JSON 格式，不要包含任何 Markdown 语法
2. 如果某个类型没有问题，不要在 issues 数组中包含该类型的条目
3. 所有字符串值都要用双引号包围
4. 数字类型（lineNumber, startLine, endLine）不要用引号
5. 确保 JSON 语法正确，注意逗号和括号的使用
